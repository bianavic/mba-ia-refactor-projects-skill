...
Ruby, PHP, C#), documentados em `references/verification-recipes.md`. O mesmo commit
acrescentou os padrões de função livre: nem toda stack expõe persistência como método de um
objeto, e um detector escrito só para o formato `Model.find(...)` aprova em silêncio um arquivo
de rotas que viola a regra. Daí a exigência, em `verification-recipes.md`, de validar todo
padrão novo contra uma violação conhecida antes de confiar num PASS. Desde então, a Fase 3 do
`SKILL.md` trata esse check como **obrigatório e nunca substituível** por teste de endpoint
(passo 6, "Static AP-16 audit").
