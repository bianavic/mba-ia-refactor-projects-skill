...
- **Execução em lote multi-repo.** A skill só rodou fora deste repositório **uma vez**: as Fases 1 e 2 contra um projeto Go/Gin público de terceiros, escolhido justamente por ser uma stack que ninguém tinha previsto ao escrever o catálogo (a Fase 3 não se aplica a código de terceiros, e o relatório dessa execução não faz parte da entrega). O teste se pagou ao quebrar a skill: o `arch-check.sh` genérico devolveu PASS em arquivos de rota cheios de violações, porque em Go a persistência é função de pacote e o padrão de detecção assumia método de receiver — corrigido no commit `a6527c1`. Uma amostra de um repositório não mede genericidade, mede sorte. O mesmo procedimento sobre N repositórios públicos produziria uma medida em vez de um argumento — e faria a lacuna equivalente numa oitava stack aparecer por processo, em vez de por acidente como foi a do Go.
